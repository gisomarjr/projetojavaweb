...
<body>
 ...
 <form action="../ControllerLoja" method="post">
              <table align="center" id="mytable" class="table table-bordred table-striped">
   ...
   **/
   ...
   <%for(Pedido pedido : p.getPedidos()){ %>
     ...
     <tr>
    <td><input type="checkbox" name="idPedido" value="<%=pedido.getId()%>" class="checkthis" /></td>
    <td><%=pedido.getId()%></td>
    <td><%=pedido.getCliente().getCpf()%></td>
    <td><%=pedido.getCliente().getNome()%></td>
   	<td><%=pedido.getData_pedido()%></td>
   	<td><%=pedido.getEntrega().getSituacao()%></td>
   	<td><%=pedido.getEntrega().getEndereco().getLogradouro()%></td>
    <td><%=pedido.getEntrega().getEndereco().getEstado()%></td>
    <td><%=pedido.getEntrega().getEndereco().getCidade()%></td>
    <td><%=pedido.getEntrega().getEndereco().getComplemento()%></td>
    <td><%=pedido.getEntrega().getEndereco().getBairro()%></td>
    <td><%=pedido.getEntrega().getEndereco().getCep()%></td>
    <!--
    ...
    <td><a class="btn btn-primary btn-xs" href="../ControllerLoja?acao=alterarSituacaoPedido"></a></td>
...
<input type="hidden" name="acao" value="alterarSituacaoPedido">
<p><input type="button" class="btn btn-primary btn-xs" value="Editar Situação do Pedido" data-title="Edit" data-toggle="modal" data-target="#edit" data-placement="top" rel="tooltip"/></p></td>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <h4 class="modal-title custom_align" id="Heading">Editar Situação do Pedido </h4>
      </div>
          <div class="modal-body">
          <div class="form-group">
      Atualize a Situação do Pedido<br>  
      <input class="form-control" name="novaSituacao" type="text" placeholder="Digite a nova Situação">
        </div>
        
        </div>
      </div>
          <div class="modal-footer ">
        <input type="submit" value="Atualizar" class="btn btn-warning btn-lg" style="width: 100%;"><span class="glyphicon glyphicon-ok-sign"></span>
      </div>
        </div>
    <!-- /.modal-content --> 
  </div>
